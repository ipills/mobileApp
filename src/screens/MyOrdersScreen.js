    ...
    db.ref()
      ...
      .then((snapshot) => {
        if (snapshot.exists()) {
          ...
          this.setState({
            orders: _orders.filter((o) => o.idUtilizador === this.state.userId),
          });
        } else {
      ...
      <View style={styles.container}>
        ...
        >
          <View>
            ...
            this.state.farmacias.length > 0 &&
            this.state.productData.length > 0 ? (
              this.state.orders.sort((a, b) => a.data < b.data).map((order, index) => {
                const farmacia = this.state.farmacias[order.idFarmacia];
                return (
                  <>
                    <View style={styles.order} key={"order_" + index}>
                      <Text style={styles.textEstablecimento}>
                        {farmacia.nome}
                      </Text>
                      <View style={styles.outros}>
                        {order.itens.map((product, index) => {
                          const _product =
                            this.state.productData[product.idProduto - 1];
                          return (
                      ...
                      <Text style={styles.textEstablecimento}>
                        Total: €{this.getTotal(order).toFixed(2)}
                      </Text>
                      <Text style={styles.textOutros}>
                        {Date(order.data).toLocaleUpperCase()}
                      </Text>
                    </View>
                  </>
                );
